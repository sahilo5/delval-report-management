...
def assembly_engineer_dashboard(request):
    ...
    # GET: render template
    return render(request, "dashboards/assembly_engineer_dashboard.html")
...
def assembler_dashboard(request):
    ...
    # Create a list with calculated pending_qty
    orders_with_pending = []
    for o in orders:
        pending_qty = o.total_qty - o.completed_qty
        # Combine material specs for display
        material = f"{o.series or ''}, {o.type or ''}, {o.size or ''}, {o.cylinder_size or ''}, {o.spring_size or ''}, {o.moc or ''}".strip(', ')
        
        # Create a new object with the calculated pending_qty
        order_with_pending = o
        order_with_pending.pending_qty = pending_qty
        order_with_pending.material = material
        orders_with_pending.append(order_with_pending)

    # Get all orders for stats
    all_orders = MainActuator.objects.all()
    total_orders = all_orders.count()
    under_assembly_count = len([o for o in orders_with_pending if o.pending_qty > 0])
    completed_count = len([o for o in orders_with_pending if o.pending_qty == 0])
    
    # Get current user's assigned orders
    my_orders = OrderDetails.objects.filter(
        order_no__in=orders,
    ...
    return render(request, "dashboards/assembler_dashboard.html", {
        "orders_under_assembly": [o for o in orders_with_pending if o.pending_qty > 0],
        "completed_orders": [o for o in orders_with_pending if o.pending_qty == 0],
        "total_orders": total_orders,
        "my_orders": my_assigned_orders,
        # For table component
        "assembly_headers": [
            {"text": "Sales Order", "key": "sales_order_no"},
            {"text": "Line Item", "key": "line_item"},
            {"text": "Order No", "key": "order_no"},
            {"text": "Customer", "key": "customer"},
            {"text": "Material", "key": "material"},
            {"text": "Quantity", "key": "order_qty"},
            {"text": "Completed", "key": "completed_qty"},
            {"text": "Pending", "key": "pending_qty"},
